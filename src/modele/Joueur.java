...
public class Joueur extends Objet implements Global {
	...
	// constantes
	private static final int MAXVIE = 100 ; // vie de départ pour tous les joueurs
		...
		switch (action) {
			...
			case TIRE :   
				if (!boule.getLabel().getjLabel().isVisible()) {
					...
					boule.tireBoule(this, lesMurs, lesJoueurs, BLAST); 
				}
				break;
			// ajout de l'ataque special | n'est disponible que 3 fois par partie
			case SPECIAL : 
				if (!boule.getLabel().getjLabel().isVisible() && this.Special < 3) {
					this.Special++;
					jeuServeur.envoi(MISSILE_S);
					boule.tireBoule(this, lesMurs, lesJoueurs, MISSILE); 
				}
				break;
		}
	 ...
	 * le missile enleve 1/3 de la vie max 
	 */
	public void perteVie(Integer Attack) {
		if(Attack == MISSILE) {
			vie = Math.max(vie-(MAXVIE/3), 0);
		}
		else {			
